...
namespace ITI.PrimarySchool.WebApp.Controllers
{
    ...
    public class GitHubController : Controller
    {
        ...
        [HttpPut("Following/{studentId}")]
        public async Task<IActionResult> FollowStudent(int studentId)
        {
            int userId = int.Parse(User.FindFirst(ClaimTypes.NameIdentifier).Value);
            Result<Student> result = await _gitHubService.FollowStudents(userId, studentId);

            return this.CreateResult<Student, FollowedStudentViewModel>(result, o =>
            {
                o.ToViewModel = s => s.ToFollowedStudentViewModel();
            });
        }
    }
}
