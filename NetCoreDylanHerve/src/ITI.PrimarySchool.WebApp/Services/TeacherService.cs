...
namespace ITI.PrimarySchool.WebApp.Services
{
    public class TeacherService
    {
        ...
        public Result<Teacher> CreateTeacher( string firstName, string lastName, int isHere )
        {
            ...
            _teacherGateway.Create( firstName, lastName, isHere);
            Teacher teacher = _teacherGateway.FindByName( firstName, lastName );
            return Result.Success( Status.Created, teacher );
        }

        public Result<Teacher> UpdateTeacher( int teacherId, string firstName, string lastName, int isHere )
        {
            if( !IsNameValid( firstName ) ) return Result.Failure<Teacher>( Status.BadRequest, "The first name is not valid." );
            if( !IsNameValid( lastName ) ) return Result.Failure<Teacher>( Status.BadRequest, "The last name is not valid." );
            Teacher teacher;
            ...
            _teacherGateway.Update( teacherId, firstName, lastName, isHere );
        ...
        public Result<IEnumerable<Teacher>> GetByClassId(int classId)
        {
            return Result.Success( Status.Ok, _teacherGateway.GetByClassId(classId));
        }
