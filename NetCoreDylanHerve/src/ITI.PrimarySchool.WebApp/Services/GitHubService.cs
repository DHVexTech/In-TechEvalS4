...
namespace ITI.PrimarySchool.WebApp.Services
{
    public class GitHubService
    {
        ...
        public async Task<Result<Student>> FollowStudents(int userId, int studentId)
        {
            User user = _userGateway.FindById(userId);
            if (user == null) return Result.Failure<Student>(Status.BadRequest, "Unknown user.");
            if (user.GithubAccessToken == string.Empty) Result.Failure<IEnumerable<Student>>(Status.BadRequest, "This user is not a known github user.");

            Student studentFollow = _studentGateway.FindById(studentId);

            _gitHubClient.FollowStudent(user.GithubAccessToken, studentFollow.GitHubLogin);

            return Result.Success(Status.Ok, studentFollow);
        }
    }
}
